...
> (2026-09-21 写回: UF-003 判 ✅已修——拍板互通性=段名版本分量、owner=(pid,starttime) 内嵌 elems 尾部、
> 池穷尽触发保守清扫(本路由+已发布+位图全死); 修法/判据/复跑见 docs/uf003_crash_reclaim_fix.md,
> 决策记录见 docs/uf003_crash_reclaim_decision.md;专项 2/2×5 轮绿, 回归面 13 驱动全绿。
> 同轮新登记 UF-013(死路由孤儿块, 清扫射程的边界)与 UF-014(clear_storage 在路由存活时调用 ⇒ 该路由析构段错误;
> 已用 /tmp/pristine 干净基线复现, 非本轮引入)。)
...
| UF-003 | chunk 池无崩溃回收 | ✅已修(owner 表=(pid,starttime) 内嵌 elems 尾部+/proc 验尸;池穷尽触发保守清扫: 本路由+已发布+位图全死才整块回收;chunk 头新增 published/route_tag;QU_CONN__ 加 __V2;顺带收口 push 失败漏块。判据 2/2×5 轮绿: 活持有者不夺→kill -9 后 reclaimed=40→多持有者只死一个不夺;回归 13 驱动全绿) | uf003_crash_reclaim_fix.md |
...
| UF-013 | 死路由孤儿块(清扫射程边界) | ⏸️边界登记: 路由段销毁后其 owner 表随之消失, 其它路由无法验证其位 ⇒ 那些块在该档位不可回收, 直到**同名路由再现**(同段名⇒同 tag⇒首次池穷尽即回收, 实测本判据每轮自愈)。影响有界(≤池容量/档), 彻底修法=owner 表按 tag 放入池段 | uf003_crash_reclaim_fix.md §5.1 |
| UF-014 | `clear_storage` 在路由存活时调用 ⇒ 该路由析构 SIGSEGV | ⬜未修(既有缺陷, **已用 /tmp/pristine 干净基线复现**, 非 UF-003 引入;矩阵: peer×payload 全组合仅 clear=yes 崩。仓库测试都在测试开头清段, 故从未暴露;档案已有"清段会打断任何在用该段的进程"注记) | uf003_crash_reclaim_fix.md §7.2 |
| DF-001~005 | DZFlat 设计待办 | ⏸️非缺陷,不进本轮修复 | dzflat_shm §9 |

## 2. 顺序与横切(0.2–0.4)

- 剩余: UF-011(⏸️判据构建待排, 其归还幂等守卫已随 94b01e2 提交);UF-005a/b+DF-*(等拍板;DF 入口=存档 0.5 第 7 条);UF-013(边界登记, 待决策是否做池段 owner 表);UF-014(既有缺陷, 待决策修不修)。
...
- UF-003(✅已修, 2026-09-21): pool_ 在共享段,`id_pool::release` 是唯一归还路径且无 owner/存活检查 ⇒ 收方被 kill -9 后其位永久悬挂, 池被吃干后该尺寸档**所有**话题永久退化为 TLV 分片(静默降级)。修法: elems 尾部内嵌 owner 表(位→(pid,starttime)),连接声明/断连清槽, 池穷尽时**保守**清扫(本路由 tag + 已发布 + 位图每位均"确定已死"才整块 CAS 归零+release, 有疑一律放弃);chunk 头空洞内加 published(1B)/route_tag(4B) 配 static_assert;`QU_CONN__` 段名加 `__V2`;顺带收口 push 失败漏块(return_unpublished)。判据 `test_uf003_crash_reclaim`(2 用例×5 轮绿, fork 崩溃臂): 活持有者⇒loan 必失败 / kill -9⇒`reclaimed=40` 后 loan 恢复且可持续 / 多持有者只死一个⇒整块放弃。回归 13 驱动全绿。残余边界: ① 死路由孤儿块(见 UF-013);② published=0 的"借出即暴死"上界 1 块/进程/档;③ /proc 隔离环境退化为不回收(无回归);④ 用户手持期与 UF-012 同契约(不设限, 靠池观测兜底)。同轮附带发现 UF-014。
