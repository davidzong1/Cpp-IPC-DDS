...
| **WHC 高低水位** | 重定标为闭环自适应限速 (AIMD, per-node) | 1 周 | 🟡 **已实现, 参数待标定** |
...
| 闭环流控 | ❌(开环限速) | 🟡 自适应限速已实现, 参数待标定 (见 §8.1) | ✅ | ✅ |
| 晚加入者补历史 | ❌ | ❌ | ❌ | ✅ |
| 第三方 DDS 互操作 | ❌ | ❌ | ❌ | ✅ |
| 标准 QoS | ❌ | ❌ | ❌ | ✅ |
...
**轻量路径闭合度 (2026-08-08): 4.5/5, 不是 5/5。** 端点分离 ✅、Heartbeat ✅、
位图 NACK ✅ 三项完整落地; NACK 抑制 🟡 (跨轮抑制已落地, 单播重传未做 —— 卡在
传输层, 见 §8.1); WHC 🟡 (重定标为闭环自适应限速, 已实现并完成标定, 但存在一个
已知的**极限环**行为, 见下)。两项 🟡 各含未闭合子项, 故 4.5/5。
...
| 重传字节占比 ≤10% | ✅ 0.27% (556 KB / 200 MB) |
| 速率收敛 | ⚠️ **未收敛到定值, 是 100M↔200M 极限环** |

延迟收益显著: 闭环 p50 9.2 ms / p99 29.9 ms, 开环 p50 21.7 ms / p99 222.6 ms
(**p99 改善 7.4 倍**) —— 这是闭环限速最实在的收益, 开环的长尾来自反复撑爆接收缓冲。

**已知行为: AIMD 极限环 (不是缺陷, 但要知道)**。速率不会停在某个定值, 而是在
100 MB/s ↔ 200 MB/s 之间往复: 连续 K 条干净消息后 ×1.5 爬升 → 爬到溢出 → 减半 →
再爬。它**不发散**(下界恒为 100 MB/s, 重传字节停止增长, NACK 停发), 属于 AIMD 的
固有形态, 与 TCP 拥塞窗口的锯齿同源。调大 K 只能拉长周期、降低 red 频率, **消除不了**
极限环 —— 根源是 ×1.5 指数爬升在 100M→200M 之间只有 2 档, 且"连续 K 条干净"这个判据
不探测溢出点。真要消除需改设计(减小增倍因子, 或做上限探测), 属 v2 范围。
...
4. ✅ 位图 NACK 真实丢包触发 (见下)。
5. ✅ 闭环自适应限速标定完成: 场景 C(开环对照) / D(僵尸隔离) / E(下限保护), 详见 §8.2 ——
   其中 4 条判据过 3 条, 未过的一条是 AIMD 极限环(已定性为固有形态而非缺陷), 另有
   一条兜底告警路径构造不可达、未记为已验证。
