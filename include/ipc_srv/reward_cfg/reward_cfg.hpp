...
public:
    ...
    void deserialize(const ipc::buffer& buffer) override
    {
        ...
        int32_t reward_cfg_names_count;
        this->adapt_memcpy_tods(reinterpret_cast<uint8_t *>(&reward_cfg_names_count), static_cast<const uint8_t *>(buffer.data()), offset, sizeof(reward_cfg_names_count));
        reward_cfg_names.clear();
        reward_cfg_names.reserve(reward_cfg_names_count);
        for (int32_t i = 0; i < reward_cfg_names_count; ++i) {
            int32_t str_size;
            ...
            reward_cfg_names.emplace_back(std::move(str));
        }
