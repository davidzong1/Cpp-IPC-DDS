...
{
    sleep<N>(k, [] { std::this_thread::sleep_for(std::chrono::milliseconds(1)); });
}
...
public:
    ...
    void unlock(void) noexcept { lc_.store(0, std::memory_order_release); }
};
...
public:
    ...
    void lock() noexcept
    {
        for (unsigned k = 0;;)
        {
            ...
            if (!old)
                return;   // got w-lock
            if (!(old & w_flag))
                break;   // other thread having r-lock
    ...
    void unlock() noexcept { lc_.store(0, std::memory_order_release); }
    ...
    void unlock_shared() noexcept { lc_.fetch_sub(1, std::memory_order_release); }
};
